...
const VENICE_MODEL = import.meta.env.VITE_VENICE_MODEL ?? 'qwen3-vl-235b-a22b';

const audioQueue: string[] = [];
let isPlayingQueue = false;

export async function analyzeFrameWithVenice(
  dataUrl: string,
  ...
  try {
    ...
    const level = String(
      parsed.threatLevel ?? 'none',
    ).toLowerCase() as ThreatLevel;
    ...
    return {
      ...
      confidence: 0,
      suggestedAction:
        'Review the raw model output and adjust the prompt if necessary.',
    };
  }
}
